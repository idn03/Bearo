...
import { getStickerForId } from "@/lib/stickers"
...
export function TodoItem({ todo, layout, onUpdate, onDelete }: TodoItemProps) {
  ...
  const sticker = getStickerForId(todo.id)
  ...
  if (layout === "card") {
    return (
      <div className="group relative flex flex-col gap-2 rounded-xl border bg-card p-4 transition-shadow hover:shadow-md">
